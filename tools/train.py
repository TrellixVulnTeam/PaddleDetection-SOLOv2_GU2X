...
def clear_model(save_dir):
    ...
    for name in path_dir:
        ...
        if sss[0] == 'best_model':   # 不会删除最优模型
            it_id = 9999999999
        elif sss[0] == 'model_final':   # 不会删除最后模型
            it_id = 9999999999
        else:
            it_id = int(sss[0])
        it_ids.append(it_id)
...
def main():
    ...
    for it in range(start_iter, cfg.max_iters):
        ...
        # ========================= eval ===========================
        if FLAGS.eval:
            if (it > 0 and it % FLAGS.eval_iter == 0 or it == cfg.max_iters - 1):
                eval_run(exe, compiled_eval_prog, eval_loader, clsid2catid, catid2name, eval_keys, eval_values, eval_cls)
                box_ap, mask_ap = eval_results(cfg.metric, cfg['EvalReader']['dataset'])
                logger.info("box ap: %.3f, mask ap: %.3f" % (box_ap[0], mask_ap[0]))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=30000,
        type=int,
        help="How many iterations to wait before model evaluation.")
    parser.add_argument(
        "--output_eval",
        default=None,
        type=str,
