...
__all__ = [
    ...
    'DeformConv', 'Conv2dUnit', 'Upsample'
]
...
class Upsample(object):
    ...
    def __call__(self, x):
        align_corners = True
        align_mode = 1
        if self.align_corners is None or self.align_corners == False:
            align_corners = False
            align_mode = 0
        if self.mode == 'bilinear':
            return L.resize_bilinear(x, out_shape=self.size, scale=self.scale_factor, align_corners=align_corners, align_mode=align_mode)
        if self.mode == 'nearest':
            return L.resize_nearest(x, out_shape=self.size, scale=self.scale_factor, align_corners=align_corners, align_mode=align_mode)
