...
class DecodeImage(BaseOperator):
    ...
    def __call__(self, sample, context=None):
        ...
        if len(gt_mask) == 0:
            ...
            gt_mask = np.zeros((img_h, img_w, 1), dtype=np.float32)
            sample['gt_bbox'] = np.array([[0, 0, 100, 100]], dtype=np.float32)
            sample['gt_score'] = np.array([[-99.0]], dtype=np.float32)
            sample['gt_class'] = np.array([[-1]], dtype=np.int32)
        else:
    ...
    def __call__(self, sample, context=None):
        ...
        if self.use_cv2:
            ...
            if 'gt_mask' in sample:
                ...
                gt_mask = np.reshape(gt_mask, (gt_mask.shape[0], gt_mask.shape[1], -1))   # 只有1个物体时通道维会不见掉。
                sample['gt_mask'] = gt_mask
            if 'semantic' in sample.keys() and sample['semantic'] is not None:
